...
class FeedbackService:
    ...
    def add_feedback(self, opportunity_id, label, notes=None):
        with self.session_factory as session:
            ...
            existing_fb = session.query(Feedback).filter(Feedback.opportunity_id == opportunity_id).first()
            if not opp:
                logger.error(f"Opportunity with id {opportunity_id} not found. Cannot add feedback.")
                return
            if existing_fb and (existing_fb.notes is not None or existing_fb.label is not None):
                logger.warning(
    ...
    def build_context(self, client, query_text, k=5):
        ...
        liked = [(f["opportunity_title"], f["feedback_notes"], ) for f in all_feedback if f["feedback_label"] == "liked"]
        rejected = [(f["opportunity_title"], f["feedback_notes"]) for f in all_feedback if f["feedback_label"] == "rejected"]
    ...
    def remove_feedback_for_opportunity(self, opportunity_id):
        with self.session_factory as session:
            feedback = session.query(Feedback).filter(Feedback.opportunity_id == opportunity_id).all()
            if feedback:
                for fb in feedback:
                    session.delete(fb)
                session.commit()
    ...
    def get_all_feedback_with_opportunities(self):
        with self.session_factory as session:
            ...
            results = []
            for fb in feedbacks:
                opp = session.query(Opportunity).filter(Opportunity.id == fb.opportunity_id).first()
                if opp:
                    results.append({
    ...
    def get_relevant_feedback(self, client, query_text, k=5):
        ...
        for f in feedbacks:
            if f.get("feedback_notes", "") is None or f.get("feedback_notes", "") == "":
                if f.get("feedback_label", "") is None or f.get("feedback_label", "") == "":
                    continue
                else:
